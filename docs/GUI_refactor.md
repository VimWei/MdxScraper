...
- 服务生命周期：在 `app.py` 创建 `SettingsService/PresetsService/ExportService` 单例并注入 `MainWindow`；页面不持有服务状态，仅通过主窗口转发。
...
- 样式集中化：将 apply_modern_styling() 拆到 theme_loader.py（Fusion 启用 + QSS）；主窗口不直接 setStyleSheet，主题在入口或 app.py 一次性加载。`theme_loader.py` 统一设置 Fusion 与 QSS，避免在多个页面重复调用 `setStyleSheet`。
- 命名与常量：输出类型、页签名称、阶段名称提取为 enums 或常量，避免硬编码重复。

### ConfigManager 使用与封装
- 当前多处直接访问 cm._config、_resolve_path 等内部属性。搬到 SettingsService 后请提供公开方法替代，避免 UI/Worker 侧越层访问。
- “关闭窗口保存编辑器内容/当前预设标签”的行为要从 MainWindow.closeEvent 迁移到 SettingsService.persist_session_state()，并在主窗口关闭时统一调用。
...
### 预设与 TOML 解析
- 将 reload_presets()/on_*_preset_changed()/on_*_save_clicked() 的 I/O 和 TOML 解析集中在 PresetsService。页面仅负责展示文本与触发保存/选择事件。
- 预设加载失败时的错误信息统一经 log_sig，不要在子线程里弹窗。

### Image 参数同步
- 把 sync_image_from_config/sync_image_to_config 的映射逻辑封装为：ImageConfig 数据类 + SettingsService.get_image_config()/update_image_config(ImageConfig)，页面只绑定控件与 DTO。

### 线程安全与信号规范
- ConversionWorker 保持无 UI 依赖，只通过 `finished_sig/error_sig/log_sig`（可加 `progress_sig(int)`）回传；日志/进度展示统一在 `CommandPanel`。
