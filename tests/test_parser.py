...
"""
    ...
    with patch.object(
        WordParser, "_open_encoding_file", return_value=mock_open(read_data=content)()
    ):
        parser = WordParser("test.txt")
        result = parser.parse()
...
def test_parse_json_file():
    ...
    with patch.object(
        WordParser, "_open_encoding_file", return_value=mock_open(read_data=json.dumps(json_data))()
    ):
...
def test_parse_xlsx_file():
    ...
    mock_sheet2.iter_rows.return_value = [[Mock(value="word4")], [Mock(value="word5")]]
...
def test_parse_xls_file():
    ...
    mock_sheet.iter_rows.return_value = [[Mock(value="word1")], [Mock(value="word2")]]
    mock_sheet.min_row = 1
