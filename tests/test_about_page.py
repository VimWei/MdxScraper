...
class TestAboutPage:
    ...
    def teardown_method(self):
        ...
        # 首先清理about_page中的线程
        if (
            hasattr(self.about_page, "version_thread")
            and self.about_page.version_thread is not None
        ):
            thread = self.about_page.version_thread
            if thread.isRunning():
                # 尝试多种退出方式
                thread.quit()  # 对于事件循环线程
                thread.requestInterruption()  # 对于自定义run方法的线程

                # 等待线程结束，增加超时时间
                if not thread.wait(3000):  # 等待3秒
                    print(f"警告: 线程 {thread} 没有在3秒内退出")
                    # 作为最后手段，强制终止（不推荐但测试中可用）
                    thread.terminate()
                    thread.wait(1000)
        ...
        # 清理跟踪列表中的所有线程
        for thread in self.active_threads[:]:  # 使用切片创建副本避免修改问题
            if thread.isRunning():
                thread.quit()
    ...
    def test_on_update_check_complete_new_version(self):
        ...
        self.about_page.on_update_check_complete(
            False, "New version 5.1.0 available. Please visit homepage to update.", "5.1.0"
        )

        # Status should be updated
        assert (
            self.about_page.update_status_label.text()
            == "New version 5.1.0 available. Please visit homepage to update."
        )
