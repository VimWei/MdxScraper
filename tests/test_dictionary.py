...
def test_dictionary_file_not_found():
    ...
    with patch(
        "mdxscraper.core.dictionary.IndexBuilder", side_effect=FileNotFoundError("File not found")
    ):
...
def test_dictionary_corrupted_file():
    ...
    with patch(
        "mdxscraper.mdict.mdict_query.IndexBuilder", side_effect=Exception("Corrupted file")
    ):
