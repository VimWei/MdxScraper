...
def test_execute_export_pdf(mock_mdx2pdf):
    ...
    result = service.execute_export(
        input_file, mdx_file, output_path, pdf_text="--page-size A4", settings_service=settings
    )
...
def test_execute_export_image(mock_mdx2img):
    ...
    result = service.execute_export(
        input_file, mdx_file, output_path, css_text="body { margin: 0; }", settings_service=settings
    )
