...
class ConversionWorker(QThread):
    ...
    def __init__(
        self, project_root: Path, cm: ConfigManager, pdf_text: str = "", css_text: str = ""
    ):
        super().__init__()
        ...
        try:
            ...
            if self.cm.get_backup_input():
                try:
                    ...
                    copy2(src, dest)
                    self.log_sig.emit(
                        f"📦 Backed up input to: {self._settings_service.to_relative(dest)}"
                    )
            ...
            if self.cm.get_save_invalid_words() and invalid_words:
                ...
                self.log_sig.emit(
                    f"📝 Invalid words saved to: {self._settings_service.to_relative(invalid_words_path)}"
                )
