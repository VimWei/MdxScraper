...
class ConfigCoordinator:
    ...
    def import_config(self, mw, file_path: Path) -> None:
        try:
            ...
            mw.log_panel.appendLog(
                f"✅ Imported config applied: {self.settings.to_relative(str(file_path))}"
            )
            if info.get("changed"):
                ...
                mw.log_panel.appendLog(
                    f"⚙️ Config normalized after import (removed: {removed}, added: {added}, type fixed: {type_fixed}). Please save to persist."
                )
    ...
    def export_config(self, mw, file_path: Path) -> None:
        try:
            ...
            new_pdf_label, new_css_label = (
                mw.preset_coordinator.create_snapshots_if_needed_on_export(mw)
            )
            if new_pdf_label:
                data.setdefault("pdf", {})["preset_label"] = new_pdf_label
                mw.log_panel.appendLog(
                    f"📌 Frozen PDF Untitled to: {self.settings.to_relative(self.presets.user_pdf_dir() / (new_pdf_label + '.toml'))}"
                )
            if new_css_label:
                data.setdefault("css", {})["preset_label"] = new_css_label
                mw.log_panel.appendLog(
                    f"📌 Frozen CSS Untitled to: {self.settings.to_relative(self.presets.user_css_dir() / (new_css_label + '.toml'))}"
                )
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(content)
            mw.log_panel.appendLog(
                f"✅ Exported config to: {self.settings.to_relative(str(file_path))}"
            )
    ...
    def restore_default_config(self, mw) -> None:
        ...
        try:
            ...
            default_config_path = (
                mw.project_root / "src" / "mdxscraper" / "config" / "default_config.toml"
            )
    ...
    # --- Advanced helpers ---
    def validate_wkhtmltopdf(
        self, path: str, force_redetect: bool = False
    ) -> tuple[bool, str, str]:
