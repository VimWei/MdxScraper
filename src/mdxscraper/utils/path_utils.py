...
def get_auto_detect_status() -> tuple[bool, str, str]:
    ...
    if _session_cache["detected_path"] is not None:
        return _session_cache["is_valid"], _session_cache["detected_path"], _session_cache["message"]
...
def validate_wkhtmltopdf_for_pdf_conversion(config_path: str) -> tuple[bool, str]:
    ...
    if not config_path or config_path in ("auto", ""):
        ...
        if detected_path == "wkhtmltopdf":
            return False, "wkhtmltopdf not found in system PATH. Please install wkhtmltopdf or specify the path manually in Advanced settings."
        config_path = detected_path
    
    # Validate the path
    is_valid, message = validate_wkhtmltopdf_path(config_path)
    if not is_valid:
        return False, f"wkhtmltopdf validation failed: {message}. Please check the path in Advanced settings."
